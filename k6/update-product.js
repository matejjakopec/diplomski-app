...
const ID         = __ENV.ID;                                   // required (string/number)
...
// ---- UPDATE BODY (arbitrary JSON) ----
let UPDATE_BODY = {};
if (__ENV.UPDATE_BODY) {
    try { UPDATE_BODY = JSON.parse(__ENV.UPDATE_BODY); } catch (_) { UPDATE_BODY = {}; }
}
...
function targetUrl() {
    if (!ID) throw new Error('Missing __ENV.ID for update');
    return `${BASE}/api/${ENGINE}/product/${encodeURIComponent(ID)}`;
}
...
export default function () {
    ...
    const res = http.patch(url, JSON.stringify(UPDATE_BODY), { headers });
